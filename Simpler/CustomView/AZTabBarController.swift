...
public class AZTabBarController: UIViewController {
    ...
    open func setButtonTintColor(color: UIColor, atIndex index: Int) {
        if !self.highlightedButtonIndexes.contains((index)) {
            ...
            button.setImage(buttonImage.withRenderingMode(.alwaysOriginal), for: .selected)
        }
    }
    ...
    private func customizeButtons(){
        for i in 0 ..< self.tabIcons.count {
            ...
            button.customizeForTabBarWithImage(self.tabIcons[i].withRenderingMode(.alwaysOriginal),
                                               highlightImage: highlightedImage?.withRenderingMode(.alwaysOriginal),
                                               selectedColor: color,
